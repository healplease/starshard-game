# Backlog — archived detail (v1–v12)

Frozen 2026-06-06 when the studio turned a new page. This holds the full per-version task tables and
the long status prose that used to live in `../shared/backlog.md`. The live board now keeps only a
one-line-per-version summary + the current game state. Nothing here is needed to build or play;
it's the detailed record of how v1–v12 were tracked.
...
### v12 — Hold-R-to-restart (mirror the Q-hold gesture on PAUSE + GAME_OVER) (shipped & passed QA, 2026-06-06) ✅
| # | Task | Owner role | Status | Artifact |
|---|------|-----------|--------|----------|
| 1 | Capture v12 feature + framing; open backlog | orchestrator | done | shared/brief.md |
| 2 | Requirements: R-hold-to-restart on PAUSE + GAME_OVER (reuse v8/v10 threshold/arc/cancel/reset); two independent hold counters (Q + R); restart hint copy updated; START/PLAY excluded; ACs | business-analyst | done | requirements/requirements/v12.md |
| 3 | GDD: confirm threshold reuse (PAUSE_QUIT_FRAMES) or set restart threshold; two-counter model + reset-on-transition for R; arc behaviour with two coexisting gestures | lead-game-designer | done | design/gdd/v12.md |
| 4 | Art spec: R-restart arc placement on PAUSE + GAME_OVER (no overlap with Q arc or text; render-smoke safe) | artist | done | art/art_spec/v12.md |
| 5 | Writer: PAUSE restart hint + GAME_OVER key-list restart clause → teach "hold R" (width-safe, no stale instant-restart copy) | writer | done | story/story/v12.md |
| 6 | Level spec: economy no-op — confirm/skip | level-designer | done | levels/level_spec/v12.md (confirmed no-op) |
| 7 | Implement R-hold counter (parallel to Q) + second arc on PAUSE + GAME_OVER (reuse v8/v10); reset both counters on transitions; scope out START/PLAY | programmer | done | game/ |
| 8 | QA: R-hold restarts from PAUSE + GAME_OVER + arc + cancel-on-release; Q and R independent; render-smoke + no v1–v11 regression | qa-tester | done | qa/qa_report/v12.md (PASS) |
| 9 | Declare v12 DONE | orchestrator | done | shared/handoffs.md |

## Status prose (archived verbatim)

- **v12 SHIPPED & DONE** (2026-06-06) — **hold-R-to-restart on PAUSE + GAME_OVER.** The human wants Restart (R)
  to wind up over a ~0.5 s hold with a progress arc, mirroring the v8/v10 Q-hold-to-quit gesture. Reuses the
  v8/v10 hold machinery (threshold `PAUSE_QUIT_FRAMES=30` + arc + cancel-on-release + reset-on-transition); the
  new wrinkle is two coexisting hold gestures (Q-quit + R-restart) on the same two screens → independent hold
  counters + two non-overlapping arcs. START/PLAY excluded (no Restart there). Orchestrator framed it
  (`shared/brief.md` v12) + opened the v12 backlog. **BA done** (`requirements/v12.md`, R83–R91 +
  AC69–AC77). **Designer done** (`design/gdd/v12.md`): threshold REUSED (`RESTART_HOLD_FRAMES =
  PAUSE_QUIT_FRAMES` = 30, coupled alias); **two independent counters** (`q_hold_frames` + new
  `r_hold_frames`, never cross-fill; quit wins a same-frame tie); **both counters reset on every
  transition** (extends the v10 §V10.4 spine); restart **moved off the `K_r` KEYDOWN edge** onto a
  held trigger (branches removed, à la v8 Esc); R arc reuses the v8 visual verbatim (idle-visibility
  matches the Q arc per screen); **active set = PAUSE + GAME_OVER only**. **Artist done**
  (`art_spec/v12.md`): R arc reuses the v8 Q arc verbatim (no new palette/arc consts), placed at the
  Q-arc centre **shifted 100 px left, same y** — `PAUSE_RESTART_ARC_CENTER=(200,483)` /
  `GAMEOVER_RESTART_ARC_CENTER=(200,545)`, each **56 px** clear of the Q arc and below every text rect;
  idle-visibility matches Q per screen (PAUSE track always-on → two empty tracks; GAME_OVER
  only-while-held). **Writer done** (`story/story/v12.md`): `PAUSE_HINT_RESTART`→"Hold R  Restart"
  + `GAMEOVER_KEYS` R clause→"Hold R  Restart      Hold Q  Quit" (Q clause kept byte-for-byte); both
  teach the hold (mirror the v8/v10 "Hold Q  Quit" shape), no stale instant-press copy, one FONT_SMALL
  line each, measured 108/221 px ≤600. **Level-designer done** (`levels/level_spec/v12.md`): **confirmed
  economy no-op** — UI-only hold gesture on PAUSE + GAME_OVER (run clock frozen/idle there), zero new
  spawn/pickup/ramp/timing; one new const is the `RESTART_HOLD_FRAMES` UI alias; the second hold counter
  (`r_hold_frames`) + its reset are UI-only; `reset_run()` unchanged; AC13 protected (START/PLAY excluded,
  R90). **Programmer done** — `r_hold_frames` counter + `_r_held()`/`_restart_hold_step()` seams, R-hold
  block in the main loop (`state in (PAUSE, GAME_OVER)`, `running`-guarded for the quit tie-break), the two
  `K_r` KEYDOWN branches removed, both counters paired-reset on all six transitions, generalised
  `hud.draw_hold_arc` + the two R arcs wired; smoke 0/120 f (×3 + `-m` + compileall + event-script), harness
  **65/65** (6 new v12). **QA PASS** (`qa/qa_report/v12.md`) — smoke exit 0 / exactly 120 f; harness 65/65;
  event-script 5/5; AC69–AC77 all PASS; render-smoke green on PAUSE + GAME_OVER with both arcs (no/Q/R/both
  held); **11 QA-authored independent probes** PASS (GAME_OVER hold-R e2e in the real loop, the #1-risk
  die-with-R-held → no instant restart / fresh 30-f hold, both-Q+R-held advance together, R-never-quits,
  exact-29/30 boundary, + a proven-FAIL negative control showing R is inert in PLAY); no AC1–AC68 regression.
  **Orchestrator declared v12 DONE.**
