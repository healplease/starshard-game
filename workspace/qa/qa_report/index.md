# QA Reports — index / navigation

> Per-run QA verdicts for "Starshard" (owner: **qa-tester**). Owner status: complete. Reports are
> **append-only, one file per increment** (this replaces the old single newest-on-top log). Each file
> records that increment's verdict, commands run, AC checklist, and regression check. Inputs are
> verified against the per-domain specs (`requirements/`, `design/gdd/`, `levels/level_spec/`,
> `art/art_spec/`, `story/story/`) + `game/` (the modular package). Cross-increment *why* → `../history.md`.

## Run reports (newest first)
| File | Increment | Verdict | Covers |
|------|-----------|---------|--------|
| `v20.md` | v20 — LASER enemy + 3-state sweeping beam + projectile ownership + death attribution | ✅ PASS | **RE-VERIFY:** AC120/R132 defect fixed — SMOKE_TIMELINE laser seed moved f1→f41 (past the f20 bomb + f40 boss-arrival flushes), so the real `App(smoke=True).run()` now reaches/persists in DAMAGING (witness `test_smoke_run_exercises_full_laser_cycle` green); full pytest **145 passed** (prior 144/1-fail → witness flipped, +1), smoke exit 0; bomb-flush + boss-clear behaviour unchanged; no AC1–AC119 regression (P1–P7 probes still hold). Prior FAIL section retained below. → orchestrator. _Original v20 detail:_ AC109–AC119 PASS via 7 independent probes (windup-harmless→lethal-at-same-spot, draw==collision negative test, frozen-target not live-track, persist-through-2-contacts to timeout, multi-laser per-source freeze via real physics, out-walkable at P_SPEED AND focused ×0.5, attribution each source + SOMETHING fallback, bomb-flush clears beams); **AC120/R132 FAIL** — smoke seeds LASER @f1 but the scripted bomb @f20 flushes `world.beams` mid-WINDUP, so the headless run NEVER reaches DAMAGING (trace `any DAMAGING==False`); smoke exit 0 + suite grew (144 pass / 1 fail = the witness), but the full-cycle-coverage clause breaks. 1-line fix: re-order SMOKE_TIMELINE laser vs bomb. → programmer |
| `v19.md` | v19 — Focus (hold-SHIFT ×0.5) + always-on small circular player hitbox + ~1.5× bullets | ✅ PASS | AC102–AC108: Focus halves both axes & reverts same frame (firing untouched, combat never reads focus); damage hitbox `P_HITBOX_R=6` (≈0.46·P_R) always-on, drawn ship unchanged; **negative test** — hazard inside old P_R but outside new circle deals 0 dmg (ebullet/asteroid/enemy/boss), core still full, pickup stays generous at P_R; every bullet family draw==collision ~1.5× (EB_R 8 measured, player 6×18→coll 9, CYAN tail 18; PELLET/NOVA/CYAN_HEAD draws retired); red indicator `#FF2840`@128 drawn at true radius 6, SHIFT+PLAY only, collision-neutral; enemy/asteroid/boss bodies unchanged; no tunnel (12.5<28); CONTROLS_1 462 px fits; full `pytest` = **117 (106→117, +10 unit/+1 e2e)** green, smoke `main.py`+`-m game.main`+compileall exit 0 (3× stable); 39/39 independent probe; no AC1–AC101 regression |
| `v18.md` | v18 — Fan side-beam 2:1 nerf + Rapid → Overdrive/Railgun (bullet speed buffable) | ✅ PASS | AC94–AC101: Fan sides 2:1 (center=100/side=50 over 1200 f), Fan rarer (12<20), Rapid retired everywhere (no kind/const/registry/ladder/pill/smoke ref), Overdrive cd 6+speed 12 / Railgun speed 16+cd 9 apply+refresh+expire→baseline+restart-reset, strongest-wins min-cd/max-speed (bounded ≥6, stepwise clean-revert), ladder sums 100 over 7 non-overlapping kinds (OD 10+RG 10=20), real-smoke Overdrive lifecycle frames 2–60→baseline; no-tunnel (16<24) + cull intact; full `pytest` = **106 (94→106, +11 unit/+1 e2e)** green, smoke exit 0 + compileall clean; no AC1–AC93 regression |
| `v17.md` | v17 — HP-bar gradient + low-HP red vignette + non-green HEAVY pellet (render-only) | ✅ PASS | V17.1 HP bar smooth green→amber→red (anchors exact, max 1-HP Δ=4.12, no step at old 40/20); V17.2 red vignette only <25 % HP, edge-only/center-clear, breathes (corner 54↔99), distinct from v6 flash (dist 271), render slot 8.5; V17.3 HEAVY pellet `#D230DC` purple (dist 256 from HP green, `EB_COLOR_GREEN` renamed), shape/size intact; full `pytest` = **94 (91→94, +3 e2e)** green, smoke `main.py`+`-m game.main`+compileall exit 0; no AC1–AC93 regression |
| `v16.md` | v16 — second boss NOVA + uniform-random extensible boss pool | ✅ PASS | AC86–AC93: extensible length-driven pool (AC86), uniform seedable pick (AC87, probe ≈½/½ over 3000 seeds), cadence unchanged (AC88), shared v7 framing (AC89), NOVA distinct (AC90), **zero ships** across a full fight (AC91, probe max enemies=0), **deadlier** every axis 25>15/80>60/90<150f (AC92); full `pytest` = **91 (75→91, +16)** green, smoke `main.py`+`-m game.main` exit 0 + compileall clean; Mothership parity + no AC1–AC85 regression (AC93) |
| `v15.md` | v15 — pytest suite + ruff/pyright tooling (port-then-delete the harness) | ✅ PASS | 32 e2e checks ported → `tests/e2e/`; full `pytest workspace/tests` = **75 (43 unit / 32 e2e)** green; smoke `main.py`+`-m game.main` exit 0 + compileall clean; parity old-75 ↔ new-75 proven; `regression_harness.py` deleted (zero coverage loss) |
| `v14.md` | v14 — one-file JSON save system + lifetime-stats screen | ✅ PASS | AC78–AC85 + STATS nav + 2 QA-authored probes (pause/resume-no-flush, idempotent double-flush) + code reads of every award/flush/runs site; real save provably untouched; harness 75/75; no AC1–AC77 regression |
| `v12.md` | v12 — hold-R-to-restart on PAUSE + GAME_OVER | ✅ PASS | AC69–AC77 + 11 QA-authored probes (GAME_OVER hold-R e2e, die-with-R-held #1-risk, both-Q+R independence, exact-29/30 boundary, negative PLAY-exclusion); harness 65/65; no AC1–AC68 regression |
| `v11.md` | v11 — softer invulnerability pulse | ✅ PASS | smooth 128↔255 cosine pulse via real-surface alpha; harness 59/59; no AC1–AC68 regression |
| `v10.md` | v10 — Q-hold-to-quit on START + GAME_OVER | ✅ PASS | AC61–AC68 + 11 QA-authored probes (exact-30 boundary, die-with-Q-held end-to-end, negative PLAY test); no v1–v9 regression |
| `v9.md` | v9 — process hardening (verification + feedback loops) | ✅ PASS | gates green + FAIL loop proven (3 planted defects) + A15 live playtest; no v1–v8 regression |
| `v8.md` | v8 — Pause / Unpause + Q-hold to Quit | ✅ PASS | AC53–AC60 + no v1/v2/v5/v6/v7 regression |
| `v7.md` | v7 — bosses / periodic Mothership boss fights | ✅ PASS | AC39–AC52 + no v1/v2/v5/v6 regression |
| `v6.md` | v6 — bombs / panic button + Z/X remap | ✅ PASS | AC30–AC38 + no v1/v2/v5 regression |
| `v5.md` | v5 — three-enemy roster + splitting pellet | ✅ PASS | AC22–AC29 + no v1/v2 regression |
| `v2.md` | v2 — bonuses + modular refactor | ✅ PASS | AC14–AC21 + no v1 regression |
| `v1.md` | v1 — base game | ✅ PASS | AC1–AC13 (AC13 tuning caveat, non-blocking) |

## Related QA docs (standing, NOT per-run — still in `../`)
- **`../test_plan.md`** — the smoke-test plan + regression-test plan + per-feature checklists.
- **`../feature_inventory.md`** — the 32-feature inventory (F1–F32) traced to R#/AC#/module.

## Updating
- **New increment:** after verifying, add `vN.md` (`## <date> (vN) — Verdict: …`) + a row above.
  One-line the FROM-qa-tester handoff in `../../shared/handoffs.md`; the *why* goes in `../history.md`.
- A **FAIL** verdict routes back to the programmer (per `CLAUDE.md`) — record it in the same `vN.md`.
